 ...
 * @package Elation
 */
define( 'ELATION_THEME_VERSION', '1.0.7' );
 ...
 * Admin notice to enter a purchase license
 */
function elation_add_launchsale_license_notice()
{
    global  $pagenow ;
    global  $current_user ;
    $elation_user_id = $current_user->ID;
    
    if ( current_user_can( 'manage_options' ) && !get_user_meta( $elation_user_id, 'elation_dismiss_launchsale_notice' ) ) {
        ?>
            <div class="notice notice-success elation-admin-notice elation-notice-add">
                <h4 style="font-size:18px;margin:12px 0 10px;"><?php 
        esc_html_e( 'Get Elation Pro on our crazy launch sale!', 'elation' );
        ?></h4>
                <p>
                    <?php 
        /* translators: %s: '20% off Elation Pro' */
        printf( esc_html__( 'Get %1$s during the launch month of September... simply use the coupon "SEPT2020" (limited offer)', 'elation' ), wp_kses( '<a href="' . esc_url( 'https://kairaweb.com/wordpress-theme/elation/' ) . '" target="_blank">' . __( '20% off Elation Pro', 'elation' ) . '</a>', array(
            'a' => array(
            'href'   => array(),
            'target' => array(),
        ),
        ...
        ?>
                ...
                <a href="?elation_add_launchsale_license_notice_ignore=" class="elation-notice-close"><?php 
        esc_html_e( 'Dismiss Notice', 'elation' );
        ?></a>
            </div><?php 
    }
...
add_action( 'admin_notices', 'elation_add_launchsale_license_notice' );
/**
 * Admin notice save dismiss to wp transient
 */
function elation_add_launchsale_license_notice_ignore()
{
    global  $current_user ;
    $elation_user_id = $current_user->ID;
    if ( isset( $_GET['elation_add_launchsale_license_notice_ignore'] ) ) {
        update_user_meta( $elation_user_id, 'elation_dismiss_launchsale_notice', true );
    }
}

add_action( 'admin_init', 'elation_add_launchsale_license_notice_ignore' );
/**
